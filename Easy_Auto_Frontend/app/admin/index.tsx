...
export default function AdminDashboard() {
  ...
  const [selectedTimeFilter, setSelectedTimeFilter] = React.useState("Last 7 Days");
    ...
    <View style={styles.container}>
      ...
      <View style={styles.subHeaderWrap}>
        <View style={styles.subHeader}>
          <Ionicons name="grid-outline" size={22} color={COLORS.primary} style={{ marginRight: 8 }} />
      ...
      <View style={[styles.bottomNav, { paddingBottom: insets.bottom + 10 }]}>
        <TouchableOpacity style={styles.bottomNavItem} onPress={() => { }}>
          ...
          <Text style={[styles.bottomNavLabel, { color: COLORS.primary, fontWeight: '700' }]}>Dashboard</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.bottomNavItem} onPress={() => router.push("/admin/ads")}>
...
function QuickActionItem({ title, description, icon, color, badge, onPress }: any) {
  return (
    <TouchableOpacity style={styles.quickActionItem} onPress={onPress}>
      ...
      <View style={styles.quickActionContent}>
        ...
        <Text style={styles.quickActionDescription} numberOfLines={1}>{description}</Text>
      </View>
...
function ActivityItem({ title, description, time, type }: any) {
  const getIcon = () => {
    if (type === 'user') return { name: 'person-circle-outline', color: '#235CF8' };

...
export default function AdminAdsScreen() {
  ...
  const handleApprove = (id: string) => {
    Alert.alert("Approve Ad", "Approve this listing?", [
      { text: "Cancel" },
      { text: "Approve", onPress: () => setAds(prev => prev.map(a => a.id === id ? { ...a, status: 'active' } : a)) }
    ]);
  };
  ...
  const handleReject = (id: string) => {
    Alert.alert("Reject Ad", "Reject this listing?", [
      { text: "Cancel" },
      { text: "Reject", style: "destructive", onPress: () => setAds(prev => prev.map(a => a.id === id ? { ...a, status: 'rejected' } : a)) }
    ]);
  };
  ...
  const handleDelete = (id: string) => {
    Alert.alert("Delete Ad", "Delete this listing permanently?", [
      { text: "Cancel" },
      { text: "Delete", style: "destructive", onPress: () => setAds(prev => prev.filter(a => a.id !== id)) }
    ]);
  };
    ...
    <View style={styles.container}>
      ...
      >
        {STATUS_FILTERS.map(f => (
          <TouchableOpacity
            key={f.key}
            style={[styles.filterChip, selectedStatus === f.key && { backgroundColor: COLORS.primary }]}
            onPress={() => setSelectedStatus(f.key as AdStatus)}
          >
            <Text style={[styles.filterText, selectedStatus === f.key && { color: COLORS.white }]}>{f.label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      ...
      <View style={[styles.bottomNav, { paddingBottom: insets.bottom + 10 }]}>
        <TouchableOpacity style={styles.bottomNavItem} onPress={() => router.push("/admin")}>
        ...
        <TouchableOpacity style={styles.bottomNavItem} onPress={() => { }}>
          ...
          <Text style={[styles.bottomNavLabel, { color: COLORS.primary, fontWeight: '700' }]}>Ads</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.bottomNavItem} onPress={() => { }}>

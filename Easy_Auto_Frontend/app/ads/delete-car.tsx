...
export default function DeleteCar() {
  ...
  const handleConfirmDelete = () => {
    Alert.alert(
      "Confirm Delete",
      "Delete this ad from active management? It will move to the Deleted section and cannot be opened again.",
      [
  ...
  const performPause = async () => {
    setPausing(true);
    try {
      const pauseRes = await api.put<{ success: boolean; message?: string }>(`/api/cars/${id}/status`, { status: 'PAUSED' });

      if (pauseRes.success) {
        Alert.alert("Paused", "The ad has been moved to the Paused section.", [
          { text: "OK", onPress: () => router.replace('/(tabs)/my-ads') }
        ]);
      } else {
        Alert.alert("Error", pauseRes.message || "Failed to pause the ad.");
      }
    } catch (error: any) {
  ...
  const performDelete = async () => {
    ...
    try {
      ...
      if (deleteRes.success) {
        Alert.alert("Deleted", "The ad has been moved to the Deleted section.", [
          { text: "OK", onPress: () => router.replace('/(tabs)/my-ads') }
        ]);
      } else {
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.pageHeader}>
          ...
          <View style={styles.headerTextWrap}>
            <Text style={styles.pageTitle}>Delete Car</Text>
            <Text style={styles.pageSubtitle}>Review the advertisement before removing it permanently.</Text>
          </View>
        </View>
